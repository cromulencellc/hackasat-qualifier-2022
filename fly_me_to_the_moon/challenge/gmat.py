...
class GmatTeplate:
    ...
    def add_maneuver( self , maneuver_string ):
        ...
        if( len(entries) != 4  ):
            print("Maneuver passed that doesnt follow format: time,dvx,dvy,dvz")
            raise ValueError
        ...
        try:
            ...
            print("Delta V entered: {} improperly formatted".format(maneuver_string))
            raise ValueError
        new_maneuver = dict()
    ...
    def render( self  , template_directory, template_filename , out_path ):
        ...
        out_text = template.render( stop_time = self.stop , manuever_list=self.data["maneuvers"] , logs=self.logs)  
