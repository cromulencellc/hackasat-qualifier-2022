...
bin_paths = [c for c in Path('/challenge/blazing_etudes').iterdir() if '' == c.suffix]
...
for c in picked:
    hash = sha256()
    with open("blazing_etudes/" + c.name, 'rb') as f:
    ...
    proc = Popen(["qemu-microblaze", "-L", "/opt/cross/microblaze-linux/", str(c)],
